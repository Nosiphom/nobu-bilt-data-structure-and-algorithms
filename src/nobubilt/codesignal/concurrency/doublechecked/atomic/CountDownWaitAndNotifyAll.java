 ...
 *
 */
public class CountDownWaitAndNotifyAll {
    private boolean isReady = false;

    public synchronized void waitForCountdown() throws InterruptedException {
        while (!isReady) {
            System.out.println(Thread.currentThread().getName() + " Waiting for countdown...");
            wait(); // Wait until notified
        }
        System.out.println(Thread.currentThread().getName() + " Countdown complete! Proceeding...");
    }
    ...
    public synchronized void startCountdown() throws InterruptedException {
        ...
        isReady = true;
        // TODO: Change notify() to notifyAll() here
        notifyAll();
        System.out.println("Countdown finished! Notified a waiting thread.");
    }
}
